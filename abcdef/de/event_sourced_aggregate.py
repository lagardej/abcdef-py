...
class EventSourcedAggregate[TState: AggregateState](EventEmittingAggregate):
    ...
    def version(self) -> int:
        ...
        Increments with each event. Used for optimistic locking,
        snapshot delta calculations, and event replay tracking.
    ...
    def load_from_state(self, state: TState) -> None:
        ...
        Paired with create_state: called by from_state() during
        reconstruction from a persisted state record. Must restore all
        domain state that create_state serialised.
