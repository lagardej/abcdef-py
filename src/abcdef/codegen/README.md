...
```
abcdef-gen module <name> --type command|query [--entrypoints ENTRYPOINT ...] [--root PATH]
abcdef-gen feature <module> <use-case> [--entrypoints ENTRYPOINT ...] [--root PATH]
```

`--root` defaults to the current working directory.

`--entrypoints` accepts one or more of `cli`, `web`, `api` (default: `cli`):
...
abcdef-gen module orders --type command --entrypoints cli web api --root src/myapp
```
...
entrypoint/cli/placeholder.py   # CLI function stub        (if cli selected)
entrypoint/web/placeholder.py   # Web handler stub         (if web selected)
entrypoint/api/placeholder.py   # API handler stub         (if api selected)
```
...
entrypoint/cli/placeholder.py   # CLI stub    (if cli selected)
entrypoint/web/placeholder.py   # Web stub    (if web selected)
entrypoint/api/placeholder.py   # API stub    (if api selected)
```
...
abcdef-gen feature orders create_order --entrypoints web api --root src/myapp
```
...
orders/entrypoint/cli/create_order.py       # CLI stub    (if cli selected)
orders/entrypoint/web/create_order.py       # Web stub    (if web selected)
orders/entrypoint/api/create_order.py       # API stub    (if api selected)
```
