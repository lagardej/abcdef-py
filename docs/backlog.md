...
- **Optimistic concurrency / version conflict detection** — `EventStore.append_events` has no `expected_version`
  parameter. Two concurrent saves for the same aggregate will silently overwrite each other. Add an `expected_version`
  parameter and raise on mismatch. This is the most critical missing correctness guarantee in the event sourcing
  implementation.
